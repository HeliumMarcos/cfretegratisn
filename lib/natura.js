...
export async function lerFrete({ fast = FAST_PADRAO, precisaLista = true } = {}) {
    ...
    if (vitrineDireta.ok && vitrineDireta.valor) {
        return {
            valor: vitrineDireta.valor,
            linhas: vitrineDireta.linhas,
            fonte: FONTE_DIRETA,
            detalheFonte: null,
            motivoProduto: null,
    ...
    const valor = extrairValor(textos).valor;
    ...
    if (!valor && linhas.length === 0 && erroVitrine) {
        throw new Error(
    ...
    return {
        valor,
        linhas: linhas.length > 0 || !valor
            ? linhas
            : [`Frete Grátis de "${MARCA_PRINCIPAL}" : ${valor}`],
        fonte: vitrine?.seletor ?? null,
        detalheFonte: null,
        motivoProduto: produto.motivo,
