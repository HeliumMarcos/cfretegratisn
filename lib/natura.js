...
export async function lerDireto() {
    ...
    const { valor } = extrairDeHtml(resultado.html);

    if (!valor) {
        return { ok: false, motivo: `sem o anúncio do limiar — leu ${amostraDoHtml(resultado.html)}` };
    }

    return { ok: true, valor, amostra: amostraDoHtml(resultado.html) };
}
...
// Lê o valor da Natura no HTML bruto, com padrão ancorado na frase inteira.
export function extrairDeHtml(html) {
    ...
    return { valor: limiar ? normalizarValor(limiar[1]) : null };
}

function comEstado(resultado) {
    const valorOk = resultado.valor !== null;
    return { ...resultado, valorOk };
}

export async function lerFrete({ fast = FAST_PADRAO } = {}) {
    ...
    if (vitrineDireta.ok && valorDireto !== null) {
        return comEstado({
            valor: valorDireto,
            fonte: FONTE_DIRETA,
            fonteValor: FONTE_DIRETA,
            detalheFonte: null,
            motivoProduto: null,
    ...
    const valor = valorBrowser;
    const fonteValor = valorBrowser !== null ? vitrine?.seletor ?? null : null;
