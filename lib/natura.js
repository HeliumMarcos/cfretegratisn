...
// 504 cru da Vercel — pior que um erro nosso, porque nao diz o motivo. Entao os limites
// sao escalonados: GOTO + SELETOR (11s) cabem dentro do timeout da requisicao (12s),
// para que o erro do Browserless chegue antes de abortarmos do nosso lado.
const GOTO_TIMEOUT = 8000;
const SELETOR_TIMEOUT = 3000;
...
const REQUISICAO_TIMEOUT = 12000;

// Margem sob os 30s: so comecamos outra tentativa se ela couber inteira aqui dentro.
const LIMITE_FUNCAO = 25000;
...
export async function buscarTextosBoxInfo({ fast = FAST_PADRAO } = {}) {
    ...
    for (const [indice, plano] of montarPlanos(fast).entries()) {
        // So comeca outra tentativa se ela couber inteira no que sobrou — comecar uma
        // que vai ser morta pela Vercel no meio troca um erro legivel por um 504 cru.
        if (indice > 0 && Date.now() + REQUISICAO_TIMEOUT > limite) {
            break;
        }
        ...
        if (!resultado.ok) {
            ...
            if (STATUS_SEM_REPETICAO.has(resultado.status)) {
                break;
            }
