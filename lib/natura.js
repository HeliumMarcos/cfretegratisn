...
export async function lerDireto() {
    ...
    const { valor, linhas } = extrairDeHtml(resultado.html);

    if (!valor && linhas.length === 0) {
        return { ok: false, motivo: `sem o anúncio do limiar — leu ${amostraDoHtml(resultado.html)}` };
    }

    return { ok: true, valor, linhas, amostra: amostraDoHtml(resultado.html) };
}
...
export async function lerLimiarProduto(limite) {
    ...
    for (const url of PRODUTO_URLS) {
        ...
        // O campo pode ser renderizado so no navegador; nesse caso ele nao esta no HTML
        // do servidor. A amostra diz qual dos dois casos e, sem precisar adivinhar.
        motivos.push(`${curto}: ${amostraDoHtml(resultado.html).slice(0, 80)}`);
    }
...
export async function buscarTextosBoxInfo({ fast = FAST_PADRAO, limite: limiteDado } = {}) {
    // Esta funcao e so o caminho do Browserless. A leitura direta acontece antes, em
    // lerFrete, que decide entre as fontes — deixar as duas aqui duplicava a decisao.
...
// O texto pode ser montado no navegador, com o HTML do servidor trazendo so o numero no
// JSON que hidrata a pagina. Exige que a chave fale de frete E de limiar — so "frete" nao
// basta, porque ha chaves de categoria e de selo que nao sao o valor que queremos — e que
// o numero seja plausivel. Sem os tres, nao devolve nada: melhor falhar que chutar.
const PADRAO_JSON_FRETE =
    /"([^"]{0,60}?(?:frete|freeshipping|shipping|freight)[^"]{0,60}?)"\s*:\s*"?(?:R\$)?\s*(\d{2,4}(?:[.,]\d{2})?)"?/gi;

const CHAVE_DE_LIMIAR = /min|valor|value|amount|limit|threshold|partir|acima|from|target/i;

export function limiarDeJson(preparado) {
    PADRAO_JSON_FRETE.lastIndex = 0;
    let match;

    while ((match = PADRAO_JSON_FRETE.exec(preparado)) !== null) {
        const chave = match[1];

        if (!CHAVE_DE_LIMIAR.test(chave)) {
            continue;
        }

        const numero = Number(match[2].replace(/\./g, '').replace(',', '.'));

        if (numero >= 10 && numero <= 1000) {
            return { valor: normalizarValor(match[2]), chave };
        }
    }
...
export function extrairLimiar(html) {
    ...
    const match = decodificado.match(PADRAO_LIMIAR);

    if (match) {
        return normalizarValor(match[1]);
    }

    return limiarDeJson(decodificado)?.valor ?? null;
}
...
export function extrairValor(textos) {
    ...
    for (const padrao of [PADRAO_VALOR_MARCA, PADRAO_VALOR, PADRAO_VALOR_INVERTIDO]) {
        for (const texto of textos) {
            ...
            if (match) {
                return { valor: normalizarValor(match[1]), textoLimpo };
            }
        }
    }
...
export function extrairLista(textos) {
    ...
    for (const texto of textos) {
        ...
        while ((match = PADRAO_LISTA.exec(texto)) !== null) {
            ...
            const linha = `Frete Grátis de "${especificacao}" : ${normalizarValor(match[2])}`;
...
// --- Leitura do HTML bruto -------------------------------------------------------
//
// Aqui NAO se tira as tags. A pagina embute HTML dentro de JSON, entao desescapar as
// aspas quebra a estrutura e o removedor de tags corta no lugar errado, deixando restos
// de classe CSS no meio do texto. Sobre essa sopa, um padrao generico casa com qualquer
// R$ da pagina — foi assim que a rota devolveu 9,00, o preco de outra coisa.
//
// A defesa e a frase inteira: "acima de" e "a partir de" so aparecem no anuncio do
// limiar. Padrao ancorado na frase nao precisa de HTML limpo para acertar.

const PADRAO_HTML_MARCA = new RegExp(
    String.raw`frete\s+gr[áa]tis\s+${MARCA_PRINCIPAL}\s+acima\s+de\s+R\$\s*(${NUMERO})`,
    'i'
);

const PADRAO_HTML_FAIXAS = new RegExp(
    String.raw`frete\s+gr[áa]tis\s+(\p{L}+)\s+acima\s+de\s+R\$\s*(${NUMERO})`,
    'giu'
);

// Prepara o HTML sem desmontar as tags: so desfaz os escapes que separam o texto do
// padrao (acentos em \uXXXX dentro do JSON, e o espaco duro entre R$ e o numero).
...
// Trecho em volta da primeira mencao a frete, para o diagnostico mostrar o que havia de
// fato no HTML. Vale muito mais que os primeiros 400 caracteres da pagina.
export function amostraDoHtml(html) {
    const preparado = prepararHtml(html);
    const posicao = preparado.search(/frete\s+gr[áa]tis/i);

    if (posicao < 0) {
        return `sem "frete grátis" em ${html.length} bytes`;
    }

    return normalizar(preparado.slice(Math.max(0, posicao - 60), posicao + 160));
}

// Lê valor e faixas do HTML bruto, só com padrões ancorados na frase inteira.
export function extrairDeHtml(html) {
    const preparado = prepararHtml(html);

    const daMarca = preparado.match(PADRAO_HTML_MARCA);
    const linhas = [];
    const vistas = new Set();

    PADRAO_HTML_FAIXAS.lastIndex = 0;
    let match;

    while ((match = PADRAO_HTML_FAIXAS.exec(preparado)) !== null) {
        const marca = match[1].charAt(0).toUpperCase() + match[1].slice(1).toLowerCase();
        const linha = `Frete Grátis de "${marca}" : ${normalizarValor(match[2])}`;
    ...
    // A faixa da marca principal primeiro, como a página que consome isto espera.
    const ehPrincipal = linha => new RegExp(`"${MARCA_PRINCIPAL}"`, 'i').test(linha);
    const ordenadas = [...linhas.filter(ehPrincipal), ...linhas.filter(l => !ehPrincipal(l))];

    // Sem a marca, aceita o limiar do texto "a partir de" (página de produto).
    const limiar = daMarca ?? preparado.match(PADRAO_LIMIAR);

    return { valor: limiar ? normalizarValor(limiar[1]) : null, linhas: ordenadas };
}

function comEstado(resultado) {
    const valorOk = resultado.valor !== null;
    const listaOk = resultado.linhas.length > 0;

    return {
        ...resultado,
        valorOk,
        listaOk,
        completo: valorOk && listaOk,
    };
}
...
export async function lerFrete({ fast = FAST_PADRAO, precisaLista = true } = {}) {
    ...
    // 1) Vitrine por GET simples. E a fonte comprovada: o anuncio das faixas esta no HTML
    //    do servidor, e o padrao ancorado na frase inteira o encontra sem ambiguidade.
    const vitrineDireta = await lerDireto();
    const valorDireto = vitrineDireta.valor ?? null;
    const linhasDiretas = vitrineDireta.linhas ?? [];

    const diretoTemOQueFoiPedido = vitrineDireta.ok && (
        precisaLista ? linhasDiretas.length > 0 : valorDireto !== null
    );

    if (diretoTemOQueFoiPedido) {
        return comEstado({
            valor: valorDireto,
            linhas: linhasDiretas,
            fonte: FONTE_DIRETA,
            fonteValor: valorDireto !== null ? FONTE_DIRETA : null,
            fonteLista: linhasDiretas.length > 0 ? FONTE_DIRETA : null,
            detalheFonte: null,
            motivoProduto: null,
            amostra: vitrineDireta.amostra,
            textos: [],
    ...
    // 2) Paginas de produto. O campo de limiar la e dedicado, mas pode ser renderizado so
    //    no navegador — nesse caso nao esta no HTML do servidor e esta etapa nao ajuda.
    const produto = valorDireto !== null
        ? { ok: false, motivo: 'não consultado — valor já encontrado na vitrine' }
        : await lerLimiarProduto(limite);

    // A pagina de produto resolve apenas o valor. Para /api/frete ela encerra a busca;
    // para /api/frete2 ainda precisamos procurar a lista completa na vitrine renderizada.
    if (produto.ok && !precisaLista) {
        return comEstado({
            valor: produto.valor,
            linhas: [`Frete Grátis de "${MARCA_PRINCIPAL}" : ${produto.valor}`],
            fonte: FONTE_PRODUTO,
            fonteValor: FONTE_PRODUTO,
            fonteLista: null,
    ...
    // 3) Browserless. Aqui o texto vem de elementos especificos, ja limpo, entao os
    //    padroes mais soltos sao seguros — o que nao seria verdade sobre o HTML bruto.
    let vitrine = null;
    ...
    const linhasBrowser = extrairLista(textos);
    const valorBrowser = extrairValor(textos).valor;
    const valor = valorDireto ?? (produto.ok ? produto.valor : null) ?? valorBrowser;
    const linhas = precisaLista
        ? linhasBrowser
        : (linhasBrowser.length > 0 || valor === null
            ? linhasBrowser
            : [`Frete Grátis de "${MARCA_PRINCIPAL}" : ${valor}`]);
    const fonteValor = valorDireto !== null
        ? FONTE_DIRETA
        : produto.ok
            ? FONTE_PRODUTO
            : valorBrowser !== null
                ? vitrine?.seletor ?? null
                : null;
    const fonteLista = linhasBrowser.length > 0 ? vitrine?.seletor ?? null : null;

    // Nenhuma fonte respondeu e houve falha de infraestrutura: isso e erro, nao "promocao
    // fora do ar". Os dois casos pedem acoes bem diferentes, e o motivo de cada fonte
    // aponta para um lugar distinto — bloqueio do site, produto sem o campo, cota.
    if (valor === null && linhas.length === 0 && erroVitrine) {
        throw new Error(
            `${erroVitrine} (leitura direta: ${vitrineDireta.motivo ?? 'sem valor'};`
            + ` páginas de produto: ${produto.motivo})`
        );
    }
    ...
    return comEstado({
        valor,
        linhas,
        fonte: precisaLista ? fonteLista ?? fonteValor : fonteValor,
        fonteValor,
        fonteLista,
        detalheFonte: fonteValor === FONTE_PRODUTO ? produto.url : null,
        motivoProduto: produto.ok ? null : produto.motivo,
        amostra: vitrineDireta.ok ? vitrineDireta.amostra : vitrineDireta.motivo,
        textos,
