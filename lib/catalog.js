...
export default async ({ page, context }) => {
  ...
  const collectOne = async (tab, job) => {
    try {
      // O slug da Natura nao e necessario para abrir um SKU. A rota direta
      // evita depender do indice de busca, que pode exibir "nenhum resultado"
      // antes da grade terminar de carregar ou ainda nao ter o item indexado.
      const directUrl = new URL('/p/-/' + job.reference, context.store_origin);
      directUrl.searchParams.set('consultoria', 'helium');
      directUrl.searchParams.set('marca', 'natura');
      await tab.goto(directUrl.href, { waitUntil: 'domcontentloaded', timeout: 22000 });
      await tab.waitForFunction((reference) => {
        const scripts = [...document.querySelectorAll('script[type="application/ld+json"]')];
        return scripts.some((script) => (script.textContent || '').toUpperCase().includes(reference));
      }, { timeout: 12000 }, job.reference).catch(() => null);
      await new Promise((resolve) => setTimeout(resolve, 500));

      let product = await tab.evaluate((reference) => {
        const priceToCents = (value) => {
          const text = String(value || '').replace(/\u00a0/g, ' ');
          const match = text.match(/(\d{1,3}(?:\.\d{3})*|\d+),([0-9]{2})/);
          return match ? Number(match[1].replace(/\./g, '')) * 100 + Number(match[2]) : null;
        ...
        for (const script of document.querySelectorAll('script[type="application/ld+json"]')) {
          try {
            ...
            schemaNodes.push(...(Array.isArray(parsed) ? parsed : [parsed]));
          } catch {}
        }
        const stack = [...schemaNodes];
        let schema = null;
        while (stack.length > 0) {
          ...
          if (types.includes('Product') && sku === reference) {
            schema = item;
            break;
          }
        ...
        const schemaPrice = Number(offer?.price ?? offer?.lowPrice);
        const promoText = document.querySelector('#product-price-por')?.textContent?.trim()
          || document.querySelector('[data-testid="product-price"]')?.textContent?.trim();
        const normalText = document.querySelector('#product-price-de')?.textContent?.trim();
        const promo = priceToCents(promoText)
          || (Number.isFinite(schemaPrice) && schemaPrice > 0 ? Math.round(schemaPrice * 100) : null);
        const normal = Math.max(priceToCents(normalText || promoText) || promo || 0, promo || 0);
        const availabilityValue = String(offer?.availability || '').toLowerCase();
        const pageText = (document.body?.innerText || '').replace(/\s+/g, ' ').toLowerCase();
        const availabilityStatus = pageText.includes('acabou, mas volta logo')
          ? 'back_soon'
          : (pageText.includes('produto esgotado') || availabilityValue.includes('outofstock')
            ? 'sold_out'
            : (availabilityValue.includes('instock') ? 'available' : 'unavailable'));
        const canonical = offer?.url || location.href;
        const sourceUrl = new URL(canonical, location.origin);
        ...
        if (!schema.name || !promo || !normal) return null;

        return {
          reference,
          name: String(schema.name).replace(/\s+/g, ' ').trim(),
          source_url: sourceUrl.href,
          image_url: imageValue ? new URL(imageValue, location.origin).href : null,
          price_normal_cents: normal,
          price_promo_cents: promo,
          available: availabilityStatus === 'available',
          availability_status: availabilityStatus,
          classification_terms: [...new Set([
            ...(document.querySelector('[data-testid="product-tags"]')?.getAttribute('data-gtm-product-tags') || '').split('|'),
            brand,
          ].filter((term) => typeof term === 'string' && term.trim().length > 0 && term.length <= 100)
            .map((term) => term.trim()))].slice(0, 10),
          sources: [{ key: 'manual_references', label: 'Busca por referências', position: 1 }],
        };
      }, job.reference);

      // Fallback para lojas em que a rota direta estiver temporariamente
      // indisponivel. Pesquisa primeiro o NATBRA e depois somente os digitos,
      // sem aceitar a mensagem provisoria de "nenhum resultado" como fim.
      for (const term of [job.reference, job.reference.replace(/^NATBRA-/, '')]) {
        if (product) break;

        const searchUrl = new URL(context.search_url);
        searchUrl.searchParams.set('busca', term);
        await tab.goto(searchUrl.href, { waitUntil: 'domcontentloaded', timeout: 22000 });
        await tab.waitForFunction((reference) => [...document.querySelectorAll('article[data-testid^="product-card-"]')]
          .some((card) => {
            const marker = (card.getAttribute('data-testid') || '').toUpperCase();
            const links = [...card.querySelectorAll('a[href*="/p/"]')]
              .map((link) => link.getAttribute('href') || '').join(' ').toUpperCase();
            return marker.includes(reference) || links.includes(reference);
          }), { timeout: 12000 }, job.reference).catch(() => null);
        await new Promise((resolve) => setTimeout(resolve, 500));

        product = await tab.evaluate((reference) => {
          const priceToCents = (value) => {
            const text = String(value || '').replace(/\u00a0/g, ' ');
            const match = text.match(/(\d{1,3}(?:\.\d{3})*|\d+),([0-9]{2})/);
            return match ? Number(match[1].replace(/\./g, '')) * 100 + Number(match[2]) : null;
          };
          const card = [...document.querySelectorAll('article[data-testid^="product-card-"]')]
            .find((item) => {
              const marker = (item.getAttribute('data-testid') || '').toUpperCase();
              const links = [...item.querySelectorAll('a[href*="/p/"]')]
                .map((link) => link.getAttribute('href') || '').join(' ').toUpperCase();
              return marker.includes(reference) || links.includes(reference);
            });
        ...
        const title = card.querySelector('[data-testid="product-card-title"]')?.textContent?.trim()
          || productLink?.getAttribute('aria-label')?.trim();
        const brand = card.querySelector('h3[aria-label^="Marca "]')?.textContent?.trim();
        const image = card.querySelector('img');
        const normalText = card.querySelector('#product-price-de')?.textContent?.trim();
        const promoText = card.querySelector('#product-price-por')?.textContent?.trim()
      ...
      else missing.push({ reference: job.reference, reason: 'Referência não encontrada após consulta direta e duas buscas na Natura.' });
    } catch (error) {
      missing.push({ reference: job.reference, reason: String(error?.message || 'Falha na pesquisa').slice(0, 220) });
    }
  };

  const tabCount = Math.min(2, queue.length);
  const tabs = [page];
...
export function extractProductClassificationFromHtml(html, reference) {
    // The schema must match the requested SKU; unrelated recommendation cards are excluded.
    if (!extractProductNameFromHtml(html, reference)) return [];
    const tag = String(html).match(/<[^>]+\bdata-testid\s*=\s*["']product-tags["'][^>]*>/i)?.[0];
    const value = tag?.match(/\bdata-gtm-product-tags\s*=\s*(["'])(.*?)\1/i)?.[2] ?? '';
    const decode = (text) => text.replace(/&quot;/g, '"').replace(/&#39;|&apos;/g, "'")
        .replace(/&amp;/g, '&').replace(/&#(\d+);/g, (_, code) => {
            const point = Number(code);
            return point > 0 && point <= 0x10ffff ? String.fromCodePoint(point) : '';
        });
    return [...new Set(decode(value).split('|').map((term) => term.trim())
        .filter((term) => term.length > 0 && term.length <= 100))].slice(0, 10);
}

export async function fetchOfficialProductData(product, fetcher = fetch) {
    for (let attempt = 1; attempt <= 2; attempt += 1) {
        try {
            ...
            if (response.ok) {
                ...
                const name = extractProductNameFromHtml(html, product.reference);
                if (name) return {
                    name,
                    classification_terms: [...new Set([
                        ...extractProductClassificationFromHtml(html, product.reference),
                        ...(product.classification_terms ?? []),
                    ])].slice(0, 10),
                };
            }
        } catch {
    ...
    return { name: product.name, classification_terms: product.classification_terms ?? [] };
}

export async function fetchOfficialProductName(product, fetcher = fetch) {
    return (await fetchOfficialProductData(product, fetcher)).name;
}
...
export function normalizeProduct(product) {
    ...
    return {
        ...
        classification_terms: [...new Set([...(product.classification_terms ?? []), product.brand].filter(Boolean))].slice(0, 10),
...
export async function attachOfficialProductData(products, workerCount = 8) {
    ...
    const workers = Array.from({ length: Math.min(workerCount, queue.length) }, async () => {
        while (queue.length > 0) {
            const product = queue.shift();
            const [details, productUrl] = await Promise.all([
                fetchOfficialProductData(product),
                shortenProductUrl(product),
            ]);
            completed.push({ ...product, ...details, product_url: productUrl });
        }
    });
...
export async function collectReferences(values) {
    ...
    const response = await fetch(`${BROWSERLESS_URL}/function?token=${encodeURIComponent(token)}`, {
        ...
        body: JSON.stringify({
            ...
            context: {
                references,
                search_url: NATURA_REFERENCE_SEARCH_URL,
                store_origin: 'https://www.minhaloja.natura.com',
            },
        }),
        signal: AbortSignal.timeout(BROWSERLESS_TIMEOUT),
    });
...
export async function lookupReferences(values) {
    ...
    const workers = Array.from({ length: Math.min(6, queue.length) }, async () => {
        while (queue.length > 0) {
            const product = queue.shift();
            const details = await fetchOfficialProductData(product);
            let productUrl = null;
            ...
            products.push({ ...product, ...details, product_url: productUrl });
        }
    });
