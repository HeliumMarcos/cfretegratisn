...
const BROWSERLESS_TIMEOUT = 24000;
...
export default async ({ page, context }) => {
  ...
  const coletar = async (aba, ranking) => {
    await preparar(aba);
    await aba.goto(ranking.url, { waitUntil: 'domcontentloaded', timeout: 14000 });
    await aba.waitForFunction(() => {
      return [...document.querySelectorAll('article[data-testid^="product-card-"]')]
        .filter((card) => [...card.querySelectorAll('a[href*="/p/"]')]
          .some((link) => /listTitle=category(?:\+|%20)page/i.test(link.getAttribute('href') || '')))
        .length >= 10;
    }, { timeout: 10000 });

    return aba.$$eval('article[data-testid^="product-card-"]', (cards, source) => cards
      .map((card) => {
  ...
  const rankings = await Promise.all([
    coletar(page, context.rankings[0]),
    coletar(secondPage, context.rankings[1]),
  ]);

  return { data: { rankings }, type: 'application/json' };
};
`;
...
export async function collectCatalog() {
    ...
    if (products.length === 0 || products.length > 20) {
        throw new Error(`quantidade inesperada de produtos coletados: ${products.length}`);
    }
